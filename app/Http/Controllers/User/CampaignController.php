...
class CampaignController extends Controller
{
    ...
    public function store(Request $request, MessageService $messageService)
    {
        try {
            ...
            $template = null;
            if ($request->has('template_id') && $request->template_id && method_exists($request->user(), 'messageTemplates')) {
                $template = $request->user()->messageTemplates()->find($request->template_id);
                if ($template) {

<div class="tab-pane fade show" id="listmessage" role="tabpanel">
    <form class="row g-3" action="{{ route('messagetest') }}" method="POST">
        ...
        <button type="button" id="add-section" class="btn btn-outline-success btn-sm mt-4">{{ __('Add Section') }}</button>
...
window.addEventListener('load', function() {
    var btnImageList = document.getElementById('image-list');
    if (btnImageList) {
        btnImageList.addEventListener('click', function(e) {
            e.preventDefault();
            var input = document.getElementById('thumbnail-list');
            window.fmSetLink = function(url) {
                if (input) { input.value = url; }
            };
            window.SetUrl = function(items) {
                if (input && items.length) {
                    input.value = items.map(function(i) { return i.url; }).join(',');
                }
            };
            window.open('{{ route("fm.fm-button") }}', 'FileManager', 'width=1000,height=500,scrollbars=yes');
        });
    }
        ...
        $('#add-section').click(function () {
            ...
            <div class="card mb-3 section" id="section${sectionIndex}" style="height: auto;">
                ...
                <div class="card-body">
                    ...
                    <button type="button" class="btn btn-outline-primary btn-sm mt-2 add-row" data-section="${sectionIndex}">{{ __('Add Row') }}</button>
                </div>
            </div>`;

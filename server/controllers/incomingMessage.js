...
const IncomingMessage = async (incomingData, socket, userIdDB, waToken, io) => {
  try {
    ...
    const { command, bufferImage, from } = await parseIncomingMessage(incomingData);
...
async function sendWebhook({ command, bufferImage, from, name, url, participant, webhook_full, data }) {
